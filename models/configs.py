...
from layers import dilated_causal_conv1d, dilated_causal_conv1d_no_pad, \
    fast_gen_dilated_causal_conv1d, self_gated_layer
    ...
    def build(self, inputs, reuse=None):
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            with tf.variable_scope("init_causal_layer"):
                ...
                # 2nd. calculate
                init_causal_out = dilated_causal_conv1d_no_pad(inputs=wav_one_hot, dilation_rate=1,
                                                               filters=self.residual_units, width=self.conv_width)
            ...
            with tf.variable_scope("dilated_causal_blocks"):
                ...
                for idx in range(self.blks):
                    with tf.variable_scope("blk_{}".format(idx)):
                        for idy in range(self.layers_per_blk):
                            with tf.variable_scope("layer_{}".format(idy)):
                                conv_out = dilated_causal_conv1d_no_pad(inputs=resi_out,
                                                                        dilation_rate=self.dilation_base ** idy,
                                                                        filters=2*self.residual_units,
                                                                        width=self.conv_width)
                                acti_out = self_gated_layer(inputs=conv_out)
                                resi_out = tf.slice(resi_out,
                                                    [0, tf.shape(resi_out)[1] - tf.shape(acti_out)[1], 0],
                                                    [-1, -1, -1]) \
                                           + tf.layers.dense(inputs=acti_out, units=self.residual_units,
                                                             activation=None, name="residual_out")
                                skip_out = tf.slice(skip_out,
                                                    [0, tf.shape(skip_out)[1] - tf.shape(acti_out)[1], 0],
                                                    [-1, -1, -1]) \
                                           + tf.layers.dense(inputs=acti_out, units=self.residual_units,
                                                             activation=None, name="skip_out")
            ...
            return {"wave": wave, "output": modes, "labels": labels, "loss": loss}


class ConfigV3(object):
    def __init__(self, name="WaveNet"):
        self.name = name
        self.max_global_steps = 200000
        self.blks = 3
